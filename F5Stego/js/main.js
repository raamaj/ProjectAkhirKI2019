...
var stegoSize = null;
    ...
    function getPSNR(){
        ...
        if ((coverHeight*coverWidth) == (stegoHeight*stegoWidth)) {
            ...
            console.log(byteCover.length);
            // while (i <= coverLength) {
            //     sumByte += bigInt(Math.pow((bigInt(byteCover[i]) - bigInt(byteStego[i])),2));
            //     i+=1;
            // }

            //var a = [1.0, 2.9, 2.8];
            //var b = [-19.8, -18.2, -10.11];
            //var MSE = mse(byteCover, byteStego)*(1/(coverHeight*coverWidth));
            //var MSE = bigInt(sumByte)/(coverHeight*coverWidth);

            var MSE = mse(byteCover,byteStego);
